...
rule all:
	input:
		...
		expand(SAM_AL_DIR + "{sample}_RNA_HISAT2_genome_aligned_gencode.sam", SAM_AL_DIR=SAM_AL_DIR, sample=SAMPLES),
		expand(BAM_AL_DIR + "{sample}_RNA_HISAT2_genome_aligned_gencode.bam", BAM_AL_DIR=BAM_AL_DIR, sample=SAMPLES),
		expand(SORTED_BAM_AL_DIR + "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup_RG.bam", SORTED_BAM_AL_DIR=SORTED_BAM_AL_DIR, sample=SAMPLES),
		expand(SORTED_BAM_AL_DIR + "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup_RG.bam.bai", SORTED_BAM_AL_DIR=SORTED_BAM_AL_DIR, sample=SAMPLES),
        expand(EXPRESSION_DIR + "{sample}_HISAT2_stranded_featurecounts_all_annotations.tsv", EXPRESSION_DIR=EXPRESSION_DIR, sample=SAMPLES)
...
rule hisat2_align_reads:
    ...
    output:
        SAM = os.path.join(SAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode.sam")
    params:
        index = "/data/CEM/shared/public_data/references/GENCODE/GRCh38.p12.genome/GRCh38.p12.genome",
        threads = 8
    run:
...
rule sam_to_bam:
	input:
		SAM = os.path.join(SAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode.sam"),
	output:
		BAM = os.path.join(BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode.bam"),
	params:
...
rule sort_bam:
    input:
        BAM = os.path.join(BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode.bam"),
    output:
        SORTED_BAM = os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord.bam"),
    params:
...
rule index_bam:
    input:
        BAM = os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord.bam"),
    output:
        os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord.bam.bai"),
...
rule mark_duplicates:
    input:
        BAM = os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord.bam")
    output:
        BAM = os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup.bam"),
...
rule readgroup:
    input:
        os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup.bam")
    output:
        os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup_RG.bam")
    params:
        sample="{sample}"
    shell:
        """
...
rule index_mkdup_bam:
	input:
		BAM = os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup_RG.bam"),
	output: 
		os.path.join(SORTED_BAM_AL_DIR, "{sample}_RNA_HISAT2_genome_aligned_gencode_sortedbycoord_mkdup_RG.bam.bai"),
	message: "Indexing sorted BAM file."
	params:
	run:
		for x in input:
...
rule feature_counts:
    input: 
        ...
        gtf = "/data/CEM/shared/public_data/references/GENCODE/gencode.v29.annotation.gene.gtf"
    output:
        counts = os.path.join(EXPRESSION_DIR, "{sample}_HISAT2_stranded_featurecounts_all_annotations.tsv")
    params:
        threads = 5
